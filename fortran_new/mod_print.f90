...
!
	...
	implicit none
	integer itertot, niter

	real aAveSec
	real auvl(nx,ny,nz), avvl(nx,ny,nz), awvl(nx,ny,nz)

	integer, private :: i, j, k, ist, gridx, gridy, gridz
...
subroutine outputres
	...
	write(9,3) timet, niter, aAveSec, itertot, resorh, resorm, &
		resoru, resorv, resorw
	write(9,5) tpeak, umax, vmax, wmax, alen, depth, width
	write(9,2) flux_north, flux_south, ahtoploss, ahtoploss, &
		flux_bottom, flux_west, flux_east, heatout, accul, &
		heatinLaser, heatvol, ratio
	write(6,3) timet, niter, aAveSec, itertot, resorh, resorm, &
		resoru, resorv, resorw
	write(6,5) tpeak, umax, vmax, wmax, alen, depth, width
	write(6,2) flux_north, flux_south, ahtoploss, ahtoploss, &
		flux_bottom, flux_west, flux_east, heatout, accul, &
		heatinLaser, heatvol, ratio

"""
Lightweight test for source term module.
"""
...
def initialize_fields(state, state_prev, mat_props, coeffs, grid):
    ...
    for i in range(ni):
        for j in range(nj):
            for k in range(nk):
                ...
                coeffs.an[i, j, k] = 1.0
                coeffs.as_[i, j, k] = 1.0
                coeffs.ae[i, j, k] = 1.0
                coeffs.aw[i, j, k] = 1.0
                coeffs.at[i, j, k] = 1.0
                coeffs.ab[i, j, k] = 1.0
                coeffs.apnot[i, j, k] = 1.0
                coeffs.su[i, j, k] = 0.0
                coeffs.sp[i, j, k] = 0.0
...
def run_source_test():
    ...
    laser_params = LaserParams(power_vol=300.0, absorptivity_vol=0.35, source_radius=30e-6, source_depth=100e-6, factor=2.0)
    laser_state = LaserState(ni, nj)
    laser_state.laser_on = True
    laser_state.beam_x = grid.x[ni // 2]
    laser_state.beam_y = grid.y[nj // 2]

    initialize_fields(state, state_prev, mat_props, coeffs, grid)

    source_term(1, state, state_prev, grid, coeffs, mat_props, laser_state, physics, sim, laser_params)
    source_term(5, state, state_prev, grid, coeffs, mat_props, laser_state, physics, sim, laser_params)

    print("source_term() executed for ivar=1 and ivar=5")
